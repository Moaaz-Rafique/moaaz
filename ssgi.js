...
async function init() {
  ...
  const objectsCycleArray = [pointLight, shortBox, tallBox, backWall, leftWall, rightWall, floor, ceiling];
  let objCycle = 0;
  ...
  window.addEventListener("keydown", function (event) {
    switch (event.key) {
      ...
      case "c":
        control.attach(
          objectsCycleArray[++objCycle % objectsCycleArray.length]
        );
      case "Escape":
        control.reset();
        break;
  ...
  window.addEventListener("resize", onWindowResize);
  const clickableObjects = [...objectsCycleArray];
  clickableObjects.push(pointLightHelper); // use helper for light
  ...
  renderer.domElement.addEventListener("pointerdown", (event) => {
    ...
    if (intersects.length > 0) {
      ...
      // if the selected is a helper for a light, attach that
      if (selected instanceof THREE.PointLightHelper) {
        control.attach(selected.light);
      } else {
